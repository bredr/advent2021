...
const getColumn = (rows: Array<string[]>, idx: number) =>
  rows.map((line) => line[idx]);

const getCounts = (x: string[]) =>
  x.reduce(
    ({ zeros, ones }, x) =>
      x === "0" ? { zeros: zeros + 1, ones } : { zeros, ones: ones + 1 },
    { zeros: 0, ones: 0 }
  );

const getMax = ({ zeros, ones }: { zeros: number; ones: number }) =>
  zeros > ones ? 0 : 1;
const getMin = ({ zeros, ones }: { zeros: number; ones: number }) =>
  zeros <= ones ? 0 : 1;
...
const getValues = (fn: (x: { zeros: number; ones: number }) => number) =>
  Array(columns)
    .fill(0)
    .map((_, idx) => fn(getCounts(getColumn(elements, idx))));
...
const getOtherValues = (fn: (x: { zeros: number; ones: number }) => number) =>
  Array(columns)
    ...
    .reduce(
      (
        { rows, values }: { rows: Array<string[]>; values: number[] },
        _,
        idx
      ) => {
        if (values !== null) {
